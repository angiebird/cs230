...
class Cityscapes(BaseDataset):
  ...
  def __getitem__(self, index):
    ...
    name = item["name"]
    image = cv2.imread(
        os.path.join(self.root, "cityscapes", item["img"]), cv2.IMREAD_COLOR)
    size = image.shape
    ...
    label = cv2.imread(
        os.path.join(self.root, "cityscapes", item["label"]),
  ...
  def multi_scale_inference(self, model, image, scales=[1], flip=False):
    ...
    final_pred = torch.zeros([1, self.num_classes, ori_height,
                              ori_width]).cuda()
    for scale in scales:
